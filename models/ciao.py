...
class LocalImplicitSRNet(nn.Module):
    def __init__(self,
        ...
        if self.non_local_attn:
            self.cs_attn = models.make(csa_spec, args={'channel': imnet_dim, 
                                                       'scale': multi_scale}) 
...
# 这个是PM版本的ciaosr
class LocalImplicitSRNetPM(nn.Module):
    def __init__(self,
                 encoder_spec,
                 imnet_q_spec,
                 imnet_k_spec,
    ...
    def query_rgb(self, features, coord, scale=None):
        ...
        return result
    def batched_predict(self, feature, coord, cell, bsize=3000):
        with torch.no_grad(): 
            n = coord.shape[1]
            left = 0
            preds = []
            while left < n:
                ...
                pred = self.query_rgb(feature, coord[:, left:right, :],
                                      cell[:, left:right, :])
                preds.append(pred)
                left = right
            pred = torch.cat(preds, dim=1)
...
# 这个是NLA版本的ciaosr
class LocalImplicitSRNetNLA(nn.Module):
    def __init__(self,
                 encoder_spec,
                 imnet_q_spec,
                 imnet_k_spec,
...
@register("ciao_pm")
class LocalImplicitSREDSRPM(LocalImplicitSRNetPM):
    """LocalImplicitSR based on EDSR.
...
@register("ciao_nla")
class LocalImplicitSREDSRNLA(LocalImplicitSRNetNLA):
    """LocalImplicitSR based on EDSR.
